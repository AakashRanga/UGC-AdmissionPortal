...
export function MysqlConfigModal({ isOpen, onClose, onConfigSaved, activeDb }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-amber-950/20 backdrop-blur-sm animate-fade-in">
      <div className="bg-white max-w-md w-full rounded-2xl p-6 border border-amber-100 shadow-2xl">
        ...
        <form onSubmit={handleConnect} className="mt-4 space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 rounded-xl border border-slate-300 text-xs font-mono text-slate-800 focus:border-blue-600 focus:ring-2 focus:ring-blue-100 outline-none"
              required
            />
          </div>
          ...
          {result && (
            <div className={`p-3 rounded-xl text-xs flex items-center gap-2 ${
              ...
              {result.status === 'success' ? <CheckCircle2 className="w-4 h-4 shrink-0 text-emerald-600" /> : <AlertCircle className="w-4 h-4 shrink-0 text-rose-600" />}
